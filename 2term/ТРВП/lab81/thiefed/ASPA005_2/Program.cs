...
using (IRepository repository = new Repository("Celebrities"))
{
    ...
    api.MapGet("/", () => repository.getAllCelebrities()); // ASPM03
    ...
    }).AddEndpointFilter<ASPA005_2.Filter.putFilter>();
    ...
    api.MapPut("/{id:int}", (int id, Celebrity celebrity) =>
    {
        ...
        if (repository.SaveChanges() <= 0) throw new SaveException("/Celebrities error, SaveChanges() <= 0");
    }).AddEndpointFilter<ASPA005_2.Filter.deleteFilter>();
    ...
    .AddEndpointFilter<ASPA005_2.Filter.addCelebrityFilter>()
    .AddEndpointFilter<ASPA005_2.Filter.getPhotoFilter>();

    api.MapFallback((HttpContext ctx) => Results.NotFound(new { error = $"Path {ctx.Request.Path} not supported" }));

    api.Map("/Celebrities/Error", (HttpContext ctx) =>
    {
        ...
        if (ex != null)
        {
            if (ex is CelebrityIdIsNullException) rc = Results.Problem(title: "ASPA004/AddCelebrity", detail: ex.Message, instance: app.Environment.EnvironmentName, statusCode: 500); // 404
...
public class CelebrityIdIsNullException : Exception { public CelebrityIdIsNullException(string message) : base($"CelebrityIsNullException: {message}") { } };
