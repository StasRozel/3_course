...
int main(int argc, char* argv[])
{
    ...
    if (!CreateProcess(
        L"E:\\Универ\\3 курс\\СП\\lab2Cmake\\build\\Debug\\lab-02x.exe 10",  // Имя исполняемого файла
    ...
    if (!CreateProcess(
        L"E:\\Универ\\3 курс\\СП\\lab2Cmake\\build\\Debug\\lab-02x.exe",  // Имя исполняемого файла
       commandLine3,               // Командная строка
        NULL,                       // Атрибуты процесса
        NULL,                       // Атрибуты потока
        FALSE,                      // Наследование дескрипторов
        0,                          // Флаги создания процесса
