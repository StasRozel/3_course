...
BEGIN
  DBMS_JOB.CHANGE(
    job       => 2,            -- ID задания
    what      => 'BEGIN job_procedure(); END;', -- Само задание
    next_date => SYSDATE + 1,   -- Следующее время выполнения (через 1 день)
    interval  => 'SYSDATE + 7'  -- Интервал повторения (каждые 7 дней)
  );
  COMMIT;
END;
