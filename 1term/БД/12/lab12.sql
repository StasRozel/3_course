...
------------------------------------ Задание №3, 4
CREATE OR REPLACE TRIGGER TASK_TABLE_BEFORE_TRIGGER
...
------------------------------------ Задание №5, 6
CREATE OR REPLACE TRIGGER TASK_TABLE_BEFORE_ROW_TRIGGER
    BEFORE INSERT OR UPDATE OR DELETE
    ON Task_Table
    FOR EACH ROW
BEGIN
...
CREATE OR REPLACE TRIGGER AUDIT_BEFORE_INSERT
    BEFORE INSERT ON AUDITS
    FOR EACH ROW
BEGIN
    DBMS_OUTPUT.PUT_LINE('Вставка строки в таблицу AUDIT');
END;
/
