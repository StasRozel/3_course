...
int main()
{
	...
	WSADATA wsaData;//буфер результат инициализации библиотеки

	try
	{
		if (WSAStartup(MAKEWORD(2, 0), &wsaData) != 0)
			throw  SetErrorMsgText("Startup:", WSAGetLastError());
	//максор описания работы с форматом сетевого адреса
	//поток или сообщение
	//дескриптор сокета
		...
		sockaddr_in s_adr;
		{
			s_adr.sin_port = htons(2000);//преобразует число в сетевой формат
			s_adr.sin_addr.S_un.S_addr = INADDR_ANY;//от каких адесов принимаем подключение
			s_adr.sin_family = AF_INET;	
		}

		if (SOCKET_ERROR == (bind(s, (LPSOCKADDR)&s_adr, sizeof(s_adr))))//связывает сокет с структурой адреса
		...
		while (true)
		{
			...
			cout << "Client IP:    " << inet_ntoa(c_adr.sin_addr) << endl;//перевод сетевого формата в строковый
